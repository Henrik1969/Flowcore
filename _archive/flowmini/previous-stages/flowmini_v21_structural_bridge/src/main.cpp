...
struct ImportExpander {
    ...
    [[nodiscard]] std::string expandRoot(const std::string& sourcePath) {
        ...
        while (std::getline(input, line) && std::getline(scanInput, scanLine)) {
            ...
            if (parseImportLine(scanLine, importText)) {
                if (sawModule) {
                    throw flow::DiagnosticError{"import", "import statements must appear before module in: " + rootPath.string()};
                }
                importExpansions.push_back(expandLibrary(canonicalImportPath(rootPath, importText)));
                continue;
            }

            if (startsWithWord(trimmed, "module")) {
                if (sawModule) { throw flow::DiagnosticError{"import", "multiple module declarations in: " + rootPath.string()}; }
                moduleLine = line;
                sawModule = true;
                continue;
            }
            ...
            if (!trimmed.empty() || sawModule) {
                rootBodyLines.push_back(line);
            }
        }

        if (!sawModule) { throw flow::DiagnosticError{"import", "root source has no module declaration: " + rootPath.string()}; }
        if (!sawMain) { throw flow::DiagnosticError{"import", "root source has no main block: " + rootPath.string()}; }
    ...
    [[nodiscard]] std::string expandLibrary(const std::filesystem::path& libPath) {
        ...
        while (std::getline(input, line) && std::getline(scanInput, scanLine)) {
            ...
            if (parseImportLine(scanLine, importText)) {
                if (sawModule) {
                    throw flow::DiagnosticError{"import", "import statements must appear before module in imported file: " + key};
                }
            ...
            if (startsWithWord(trimmed, "module")) {
                if (sawModule) { throw flow::DiagnosticError{"import", "multiple module declarations in imported file: " + key}; }
                sawModule = true;
                continue;
            }

            if (startsWithWord(trimmed, "main")) {
                throw flow::DiagnosticError{"import", "imported file defines main; libraries may only provide reusable declarations: " + key};
            }
        ...
        if (!sawModule) { throw flow::DiagnosticError{"import", "imported file has no module declaration: " + key}; }
...
void printUsage(std::ostream& out) {
    out
        << "Usage:\n"
        << "  flowmini [--trace true|false] [--emit-flowir <file|->] [--dump-token-tree <file|->] [--dump-symbols <file|->] <module.flow|module.flowir> < input\n\n"
