...
public:
    ...
    std::vector<Route> run(MiniEnvelope env) override {
        ...
        setPathValue(record, out, list.at(checkedIndex(rawIndex, list.size(), "list.get")), "list.get");
        return {Route{"out", std::move(env)}};
    }
};
...
public:
    ...
    std::vector<Route> run(MiniEnvelope env) override {
        const auto& record = requirePayloadConst<RecordPayload>(env, "list.print_int_lines");
        const std::string listPath = getPolicyString(config_, "list", "list");
