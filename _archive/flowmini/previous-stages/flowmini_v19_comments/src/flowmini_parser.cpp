...
enum class ExprKind { LiteralInt, LiteralString, Identifier, StdinInt, ListIndex, ArrayIndex, FieldAccess, ListLength, FunctionCall, Binary };
struct Expr {
...
private:
    ...
    [[nodiscard]] bool isCallableType(const std::string& type) const {
        return isIntLikeType(type) || type == "c_string" || isAbiStructType(type) || isRecordType(type);
    }
