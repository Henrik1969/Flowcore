...
public:
    ...
    std::vector<Route> run(MiniEnvelope env) override {
        ...
        if (convention != "c") {
            throw flow::DiagnosticError{"abi.call", "only C calling convention is supported in flowmini v15"};
        }
        ...
        if (argTypes.size() == 1 && argTypes[0] == "c_string" && returnType == "c_size_t") {
            ...
            throw flow::DiagnosticError{"abi.call", "unsupported v15 ABI signature: " + returnType + "(" + argTypesText + ")"};
        }
