...
enum class ExprKind { LiteralInt, Identifier, StdinInt, ListIndex, ArrayIndex, ListLength, FunctionCall, Binary };
struct Expr {
...
private:
    ...
    [[nodiscard]] std::string exprType(const Expr& expr) const {
        ...
        if (expr.kind == ExprKind::ListIndex) {
            ...
            if (exprType(*expr.left) != "int") { throw flow::DiagnosticError{"lowerer", "index for '" + expr.ident + "' must be int"}; }
        ...
        if (expr.kind == ExprKind::ArrayIndex) {
            ...
            for (const auto& index : expr.indices) { if (exprType(index) != "int") { throw flow::DiagnosticError{"lowerer", "array indexes for '" + expr.ident + "' must be int"}; } }
            return "int";
        }
        if (expr.kind == ExprKind::FunctionCall) {
            ...
            for (std::size_t i = 0; i < expr.args.size(); ++i) {
                ...
                if (actual != fn->args[i].type) { throw flow::DiagnosticError{"lowerer", "argument " + std::to_string(i + 1) + " for function '" + expr.ident + "' expects " + fn->args[i].type + " but got " + actual}; }
            }
    ...
    [[nodiscard]] std::string targetType(const Target& target) const {
        ...
        if (target.kind == Target::Kind::ListIndex) {
            if (exprType(*target.index) != "int") { throw flow::DiagnosticError{"lowerer", "assignment index for '" + target.ident + "' must be int"}; }
            if (sym->type == "list<int>") { return "int"; }
            if (isArrayIntType(sym->type)) {
        ...
        for (const auto& index : target.indices) { if (exprType(index) != "int") { throw flow::DiagnosticError{"lowerer", "array assignment indexes for '" + target.ident + "' must be int"}; } }
        return "int";
    }
    ...
    [[nodiscard]] Step lowerFunctionCall(const Expr& expr, const std::string& requestedOutPath) {
        ...
        for (std::size_t i = 0; i < expr.args.size(); ++i) {
            if (exprType(expr.args[i]) != fn->args[i].type) { throw flow::DiagnosticError{"lowerer", "argument type mismatch for function '" + expr.ident + "'"}; }
            Step argStep;
    ...
    void parseFunctionDecl() {
        ...
        if (!check(TokenKind::RightParen)) {
            while (true) {
                ...
                if (argType != "int") { fail(argToken, "flowmini v12 function arguments only support int"); }
        ...
        if (def.returnType != "int") { fail(nameToken, "flowmini v12 functions only support int return type"); }
