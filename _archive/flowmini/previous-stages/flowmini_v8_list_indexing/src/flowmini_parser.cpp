...
private:
    ...
    [[nodiscard]] std::string lowerExprToPath(const Expr& expr, const std::string& targetHint, Step* step) {
        ...
        if (expr.kind == ExprKind::ListIndex) {
            ...
            appendStep(combined, Step{false, {id, "in"}, {id, "out"}});
    ...
    [[nodiscard]] Step parsePrintStatement() {
        ...
        if (type == "list<int>" && expr.kind == ExprKind::Identifier) {
            ...
            return Step{false, {id, "in"}, {id, "out"}};
        }
    ...
    [[nodiscard]] Step parseIfStatement() {
        ...
        Step start = condStep;
        appendStep(start, Step{false, {routeId, "in"}, {routeId, "false"}});

        enterScope("if" + std::to_string(ifIndex) + "_true");
        Step trueBody = parseBlockStatementsUntilRightBrace();
        leaveScope();
        if (trueBody.empty) { fail(peek(), "if true block may not be empty in flowmini v7"); }
        ...
        if (hasElse) {
            ...
            leaveScope();
            if (falseBody.empty) { fail(peek(), "else block may not be empty in flowmini v7"); }
        }
        ...
        addWire(trueBody.last, {joinId, "in"});

        if (hasElse) {
            addWire({routeId, "false"}, falseBody.first);
            addWire(falseBody.last, {joinId, "in"});
        } else {
            addWire({routeId, "false"}, {joinId, "in"});
        }

        return Step{false, start.first, {joinId, "out"}};
    }
