...
struct Value {
    using Storage = std::variant<std::monostate, bool, std::int64_t, std::string, Record, List, Array, AbiStruct>;
    Storage data;
    ...
    Value(AbiStruct value) : data(std::move(value)) {}
};
...
[[nodiscard]] const AbiStruct& getPathAbiStruct(const RecordPayload& payload, const std::string& path, const std::string& stage);
