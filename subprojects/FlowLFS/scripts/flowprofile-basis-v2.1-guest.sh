#!/usr/bin/env bash
set -euo pipefail
profile=basis-v2.1; state_root=/flow/store/profile-realizations/$profile
packages=(zsh flow-basis flow-terminal sel)
versions=(5.9.2 0.1.0 1.0.0 0.2.1)
objects=(
 ...
 sha256-4f15abfd0db057e5fc5bcf3d230b7a34ce87476bdfe51f95b686c5d57401c535
)
die(){ printf 'FLOWPROFILE_BASIS_V21_ERROR %s\n' "$*" >&2; exit 1; }
test "$(id -u)" -eq 0||die 'owner projection authority requires root'
transaction(){ case "$1" in zsh) /usr/local/sbin/flowpkg-zsh "$2";; flow-basis) /usr/local/sbin/flowpkg-basis "$2";; flow-terminal) /usr/local/sbin/flowpkg-flow-terminal "$2";; sel) /usr/local/sbin/flowpkg-sel-v0.2.1 "$2";; esac; }
