#!/usr/bin/env bash
set -euo pipefail
profile=selection-providers-v1; state=/flow/store/profile-realizations/$profile; oldstate=/flow/store/profile-realizations/selection-providers-v0
packages=(go fzf flow-selection); versions=(1.26.5 0.74.1 0.1.1)
objects=(sha256-31c8d748b54d460c4d114fcd9e94e551164cd109cdee52ddfd74bd3b1b503652 sha256-205dea12964e9664feaeb2987c815d15fbf9ace1fad3bd49ccb34708c8eebf7f sha256-ef7c586436891172f0c820fbc507204646e1561d2e908f5797e8c58a7a6c9199)
...
activate(){ preflight; mkdir -p "$state"; :>"$state/active.tsv.incoming"; for i in "${!packages[@]}"; do p=${packages[$i]}; v=${versions[$i]}; test -d "/flow/store/projections/$p-$v"||tx "$p" project; tx "$p" verify; printf '%s\t%s\t%s\n' "$p" "$v" "${objects[$i]}">>"$state/active.tsv.incoming"; done; mv "$state/active.tsv.incoming" "$state/active.tsv"; if test -f "$oldstate/active.tsv"; then mv "$oldstate/active.tsv" "$oldstate/superseded-by-v1.tsv"; fi; echo 'FLOWPROFILE_SELECTION_ACTIVATE_PASS default=native native_projection=full-list'; }
...
verify(){ preflight; test -f "$state/active.tsv"; /usr/local/sbin/flowpkg-go verify; /usr/local/sbin/flowpkg-fzf verify; /usr/local/sbin/flowpkg-flow-selection verify; systemctl is-active --quiet sshd.service; echo 'FLOWPROFILE_SELECTION_VERIFY_PASS providers=native,fzf default=native native_projection=full-list'; }
