...
grep -q '"lowering_profile": "none"' "$tmpdir/kernel-random.semantic.json"
...
grep -q '"lowering_profile": "none"' "$tmpdir/kernel-openat.semantic.json"
...
grep -q '"lowering_profile": "none"' "$tmpdir/kernel-read.semantic.json"
...
grep -q '"lowering_profile": "none"' "$tmpdir/kernel-write.semantic.json"
...
grep -q '"lowering_profile": "none"' "$tmpdir/kernel-lseek.semantic.json"
...
grep -q '"lowering_profile": "none"' "$tmpdir/kernel-unlinkat.semantic.json"
...
jq -e '.lowering_profile == "none" and ([.lowering_plan.operations[] | select(.kind == "loop")] | length) == 2 and any(.lowering_plan.operations[]; .kind == "assignment") and any(.lowering_plan.operations[]; .kind == "external_call" and .provider.symbol == "sendfile")' "$tmpdir/flowcat.semantic.json" >/dev/null
