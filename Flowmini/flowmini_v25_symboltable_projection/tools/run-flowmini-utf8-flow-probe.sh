...
jq -e '.status == "ok" and any(.variant_carriers[]?.members[]?; .member == "scalar" and .discriminant == 0)' "${tmpdir}/variant-when-semantic.json" >/dev/null
...
jq -e '.status == "ok" and any(.variant_carriers[]?.members[]?; .member == "diagnostic" and .discriminant == 1)' "${tmpdir}/variant-diagnostic-semantic.json" >/dev/null
