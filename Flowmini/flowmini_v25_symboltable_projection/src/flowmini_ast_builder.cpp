...
namespace flowmini::ast {
    ...
    namespace {
        ...
        bool is_enum_token(const flowmini::Token& token) {
        ...
        bool is_expression_boundary_token(const flowmini::Token& token) {
            return token.kind == flowmini::TokenKind::Newline ||
                   ...
                   token.kind == flowmini::TokenKind::KeywordElse ||
    ...
    std::size_t add_expression_placeholder_at(std::vector<Expression>& expressionPool,
        ...
        bool is_guard_token(const flowmini::Token& token) {
        ...
        std::size_t parse_guard_statement_shell(const std::vector<flowmini::Token>& tokens,
            ...
            if (has_expression_until_body_or_line_end(tokens, i)) {
                conditionExpression = add_expression_placeholder_at(expressionPool, tokens, i);
            }
            i = skip_until_body_block_or_line_end(tokens, i);
    ...
    AstModule build_source_header_ast(const std::vector<flowmini::Token>& tokens) {
        ...
        while (i<tokens.size() && !is_end_token(tokens[i])) {
            ...
            if (is_enum_token(tokens[i])) {
                EnumDecl enumDecl;
                enumDecl.location = location_from_token(tokens[i]);
                ++i;
                if (i < tokens.size() && tokens[i].kind == flowmini::TokenKind::Identifier) {
                    enumDecl.name = tokens[i].text;
                    ++i;
                }
                if (i < tokens.size() && tokens[i].kind == flowmini::TokenKind::LeftBrace) {
                    ++i;
                    while (i < tokens.size() && !is_end_token(tokens[i]) &&
                        ...
                        if (tokens[i].kind == flowmini::TokenKind::Identifier) {
                            enumDecl.members.push_back(EnumMember{tokens[i].text, location_from_token(tokens[i])});
                        }
                        ++i;
                    }
