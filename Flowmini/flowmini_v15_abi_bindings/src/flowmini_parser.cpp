...
enum class ExprKind { LiteralInt, LiteralString, Identifier, StdinInt, ListIndex, ArrayIndex, ListLength, FunctionCall, Binary };
struct Expr {
...
struct Target {
    enum class Kind { Identifier, ListIndex, ArrayIndex } kind = Kind::Identifier;
    std::string ident;
...
private:
    ...
    [[nodiscard]] bool isCAbiType(const std::string& type) const {
        return type == "c_int" || type == "c_long" || type == "c_size_t" || type == "c_string";
    }

    [[nodiscard]] bool isCallableType(const std::string& type) const {
        return isIntLikeType(type) || type == "c_string";
    }
        ...
        if (check(TokenKind::Identifier)) {
            ...
            if (match(TokenKind::LeftParen)) {
                ...
                if (!check(TokenKind::RightParen)) {
                    while (true) {
                        expr.args.push_back(parseValueExpr());
    ...
    [[nodiscard]] std::string lowerExprToPath(const Expr& expr, const std::string& targetHint, Step* step) {
        ...
        if (expr.kind == ExprKind::ListIndex) {
            ...
            const std::string id = generatedId(sym->type == "list<int>" ? "list_get" : "array_get");
            const std::string out = targetHint.empty() ? generatedId("tmp") : targetHint;
            addNode("node", id, sym->type == "list<int>" ? "list.get" : "array.get");
            if (sym->type == "list<int>") { addPolicy(id, "list", sym->path); addPolicy(id, "index", indexPath); }
            else { addPolicy(id, "array", sym->path); addPolicy(id, "indices", indexPath); }
        ...
        if (expr.kind == ExprKind::ArrayIndex) {
            ...
            addPolicy(id, "array", arraySym->path); addPolicy(id, "indices", paths.str()); addPolicy(id, "out", out);
            appendStep(combined, Step{false, false, {id, "in"}, {id, "out"}});
            if (step != nullptr) { appendStep(*step, combined); }
            return out;
        }
    ...
    void parseTypeDecl() {
        ...
        const Token& refinesToken = expectIdentifier("expected 'refines' after type name");
        if (refinesToken.text != "refines") { fail(refinesToken, "expected 'refines' after type name"); }
        ...
        while (!check(TokenKind::RightBrace) && !check(TokenKind::End)) {
            const Token& invToken = expectIdentifier("expected 'invariant' in type body");
            if (invToken.text != "invariant") { fail(invToken, "flowmini v14 type blocks only support invariant declarations"); }
    ...
    void parseAbiBlock() {
        ...
        while (!check(TokenKind::RightBrace) && !check(TokenKind::End)) {
            ...
            if (head.text == "convention") {
                ...
                if (convention != "c") { fail(head, "flowmini v15 supports only C ABI convention"); }
            ...
            fail(head, "expected library, convention, or extern in abi block");
        }
    ...
    void parseExternFunctionDecl(const std::string& abiName, const std::string& library, const std::string& convention) {
        ...
        if (!check(TokenKind::RightParen)) {
            while (true) {
                ...
                if (!isCAbiType(argType)) { fail(argToken, "flowmini v15 extern arguments require c_int, c_long, c_size_t, or c_string"); }
        ...
        if (!(def.returnType == "c_int" || def.returnType == "c_long" || def.returnType == "c_size_t")) { fail(nameToken, "flowmini v15 extern return type must be c_int, c_long, or c_size_t"); }
        ...
        while (!check(TokenKind::RightBrace) && !check(TokenKind::End)) {
            ...
            if (prop.text == "symbol") {
                ...
                fail(prop, "flowmini v15 extern body supports only symbol and effect");
            }
    ...
    void parseFunctionDecl() {
        ...
        if (!check(TokenKind::RightParen)) {
            while (true) {
                ...
                if (!isIntLikeType(argType)) { fail(argToken, "flowmini v14 function arguments currently require an int-like type"); }
        ...
        if (!isIntLikeType(def.returnType)) { fail(nameToken, "flowmini v14 functions currently require an int-like return type"); }
    ...
    [[nodiscard]] Step lowerAssignmentToTarget(const Expr& expr, const Target& target) {
        ...
        if (target.kind == Target::Kind::ListIndex) {
            ...
            const bool isList = containerSym->type == "list<int>";
            const std::string id = generatedId(isList ? "list_set" : "array_set");
            addNode("node", id, isList ? "list.set" : "array.set");
            if (isList) { addPolicy(id, "list", containerSym->path); addPolicy(id, "index", indexPath); }
            else { addPolicy(id, "array", containerSym->path); addPolicy(id, "indices", indexPath); }
        ...
        addPolicy(id, "array", containerSym->path); addPolicy(id, "indices", paths.str()); addPolicy(id, "value", valuePath);
        appendStep(combined, Step{false, false, {id, "in"}, {id, "out"}});
        return combined;
    }
    ...
    [[nodiscard]] Step parsePrintStatement() {
        ...
        throw flow::DiagnosticError{"lowerer", "print currently supports int expressions, list<int> identifiers, and array<int> identifiers only"};
    }
