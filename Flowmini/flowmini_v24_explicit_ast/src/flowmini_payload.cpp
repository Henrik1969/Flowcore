...
const char* valueTypeName(const Value& value) {
    ...
    if (std::holds_alternative<Array>(value.data)) { return "Array"; }
    if (std::holds_alternative<AbiStruct>(value.data)) { return "AbiStruct"; }
    return "unknown";
}
...
const Array& getPathArray(const RecordPayload& payload, const std::string& path, const std::string& stage) {
    const Value& value = getPathValue(payload, path, stage);
    if (const auto* typed = std::get_if<Array>(&value.data)) {
        return *typed;
    }
    throw flow::DiagnosticError{stage, std::string{"record path '"} + path + "' expected Array, got " + valueTypeName(value)};
}

Array& getPathArrayMutable(RecordPayload& payload, const std::string& path, const std::string& stage) {
    Value& value = getPathValueMutable(payload, path, stage);
    if (auto* typed = std::get_if<Array>(&value.data)) {
        return *typed;
    }
    throw flow::DiagnosticError{stage, std::string{"record path '"} + path + "' expected Array, got " + valueTypeName(value)};
}
...
const AbiStruct& getPathAbiStruct(const RecordPayload& payload, const std::string& path, const std::string& stage) {
    const Value& value = getPathValue(payload, path, stage);
    if (const auto* typed = std::get_if<AbiStruct>(&value.data)) {
        return *typed;
    }
    throw flow::DiagnosticError{stage, std::string{"record path '"} + path + "' expected AbiStruct, got " + valueTypeName(value)};
}

void setPathAbiStruct(RecordPayload& payload, const std::string& path, AbiStruct value, const std::string& stage) {
    setPathValue(payload, path, Value{std::move(value)}, stage);
}
