...
private:
    ...
    [[nodiscard]] bool isCAbiType(const std::string& type) const {
        return type == "c_int" || type == "c_long" || type == "c_size_t" || isAbiPointerType(type);
    }
    ...
    void parseAbiBlock() {
        ...
        while (!check(TokenKind::RightBrace) && !check(TokenKind::End)) {
            ...
            fail(head, "expected library, convention, type, or extern in abi block");
        }
    ...
    void parseExternFunctionDecl(const std::string& abiName, const std::string& library, const std::string& convention) {
        ...
        if (!check(TokenKind::RightParen)) {
            while (true) {
                ...
                if (!isCAbiType(argType)) { fail(argToken, "flowmini v16 extern arguments require c_int, c_long, c_size_t, or a declared ABI pointer-shaped type"); }
        ...
        if (!(def.returnType == "c_int" || def.returnType == "c_long" || def.returnType == "c_size_t")) { fail(nameToken, "flowmini v16 extern return type must be c_int, c_long, or c_size_t"); }
        ...
        while (!check(TokenKind::RightBrace) && !check(TokenKind::End)) {
            ...
            if (prop.text == "symbol") {
                ...
                fail(prop, "flowmini v16 extern body supports only symbol and effect");
            }
